...
clear
clc
pt = 'E:\code\matlab\车牌\getword\data\';
ext = '*.png';
dis = dir([pt ext]);
nms = {dis.name};
...
for k = 1:1:length(nms)
  ...
  img = RemoveEdge(img);
   ...
   index = 1;%0.85*40=34，1.15*40=46
    ...
    savedir = nms{k}(1:length(nms{k})-4);
     savedir = strcat('E:\code\matlab\车牌\getword\TEST\',savedir);
    mkdir(savedir);
    for i=1:length(Letter)
            LL_name=strcat('SegLetter',num2str(i),'.png');
             %if  Letter{i}~=null
                imwrite(Letter{i},[savedir,'\',LL_name]);
            % end
    end
end
